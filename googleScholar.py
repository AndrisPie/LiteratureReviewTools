...
class ScholarConf(object):
    ...
    VERSION = '2.10'
    LOG_LEVEL = 1
    MAX_PAGE_RESULTS = 1000 # Current maximum for per-page results
...
class ScholarQuerier(object):
    ...
    def send_query(self, query,start):
        """
        ...
        html = self._get_http_response(url=query.get_url()+'&start='+str(start),
...
def googleScholarSearch(searchTerms,numRes,saveDir):
   
    # Create Query and Querier class 
    querier = ScholarQuerier()
    query = SearchScholarQuery();
    
    # Assign input to the query class
    query.set_words(searchTerms)    
    ...
    for it in range(numLoops):
        
        start = it*20;
        querier.send_query(query,start)
